...
function getSpouseTaxAllowance() public view returns (uint) {
    require(isMarried, "Not married");
    require(spouse != address(0), "No spouse");

    return Taxpayer(spouse).getTaxAllowance();
}
...
function setTaxAllowanceBySpouse(uint ta) public {
 ...
 function setTaxAllowance(uint ta) public {
  ...
  function joinLottery(address lot, uint256 r) public onlyOwner {
    ...
    require(age < 65, "Too old");
    require(lot != address(0) && lot.code.length > 0, "Invalid lottery");

    Lottery(lot).commit(keccak256(abi.encode(r)));
    rev = r;
    committed = true;
}
